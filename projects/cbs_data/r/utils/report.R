# utils/report.R
# --------------------------------------------------------------
# Bouwt het volledige HTML-rapport: CSS/typografie (uit parameters.R),
# narratieve tekst en de interactieve Plotly-grafieken uit visuals.R.
#
# Dit project gebruikt bewust een los R-script in plaats van R Markdown:
# knitr's eigen HTML-opmaak zou botsen met de custom lay-out/CSS hieronder.
# fig_to_html() doet daarom vanuit een gewone R-list (zie visuals.R) precies
# wat pio.to_html() in de Python-versie doet: de data/layout serialiseren
# naar JSON en zelf de <div>/<script>-tags eromheen bouwen, met Plotly.js
# 1x via het CDN geladen (bij de eerste grafiek).

fig_to_html <- function(fig, div_id, include_js = FALSE) {
  data_json <- jsonlite::toJSON(fig$data, auto_unbox = TRUE, null = "null", digits = NA)
  layout_json <- jsonlite::toJSON(fig$layout, auto_unbox = TRUE, null = "null", digits = NA)

  cdn_script <- if (include_js) {
  ...
  sprintf(
    '%s<div id="%s" class="plotly-graph-div" style="height:100%%; width:100%%;"></div>
<script>
  Plotly.newPlot("%s", %s, %s, {displayModeBar: false, responsive: true});
</script>',
    cdn_script, div_id, div_id, data_json, layout_json
  )
}

# Simpele {{TOKEN}}-vervanging voor het HTML-sjabloon hieronder. Plotly's
# eigen hover-syntax gebruikt "%{...}" (met een procentteken), dus die
# botst niet met onze "{{...}}"-tokens.
render_template <- function(template, vars) {
  for (key in names(vars)) {
    token <- paste0("{{", key, "}}")
    template <- gsub(token, as.character(vars[[key]]), template, fixed = TRUE)
  }
  template
}
...
', INK, PAPER, PAPER_RAISED, GRID, GOLD, TEAL, WINE, MUTED,
   FONT_SERIF, FONT_SANS)

HOOFDSTUKKEN <- list(
  c("inleiding", "Inleiding"),
  c("overledenen", "Aantal overledenen"),
  c("leeftijd", "Leeftijdsopbouw"),
  c("levensverwachting", "Levensverwachting"),
  c("eu", "Nederland in Europa"),
  c("gestandaardiseerd", "Gestandaardiseerde sterfte"),
  c("scenario", "Scenario: zonder de pandemie"),
  c("methode", "Methode en bronnen")
)

# Bouw het volledige HTML-rapport.
#   datasets: output van data_loader::load_all()
#   results:  output van analysis::run_all(datasets)
build_report <- function(datasets, results) {
  cijfers <- results$kerncijfers
  aandelen <- results$leeftijdsaandelen
  eu <- results$eu_ranking
  scenario <- results$scenario_sterfte

  fig1 <- fig_sterfte_drieluik(datasets$sterftetrends)
  fig2 <- fig_leeftijdsopbouw(aandelen)
  fig3 <- fig_levensverwachting(datasets$levensverwachting)
  fig4 <- fig_eu_vergelijking(eu)
  fig5 <- fig_gebeurtenissen_annotatie(datasets$sterftetrends, datasets$gebeurtenissen)
  fig6 <- fig_scenario(scenario)

  toc_html <- paste0(
    vapply(HOOFDSTUKKEN, function(h) sprintf('<li><a href="#%s">%s</a></li>', h[1], h[2]),
           character(1)),
    collapse = "\n"
  )
  ...
  vars <- list(
    ...
    CSS = CSS,
    TOC_HTML = toc_html,
    VANDAAG = format(Sys.Date(), "%-d %B %Y"),
    LAATSTE_JAAR = cijfers$laatste_jaar,
    OVERLEDENEN_LAATSTE = fmt(cijfers$overledenen_laatste, 0),
    OVERLEDENEN_VERSCHIL = fmt(cijfers$overledenen_verschil * 1000, 0),
    LV_MANNEN_LAATSTE = fmt(cijfers$lv_mannen_laatste, 1),
    LV_VROUWEN_LAATSTE = fmt(cijfers$lv_vrouwen_laatste, 1),
    LV_MANNEN_2019 = fmt(cijfers$lv_mannen_2019, 2),
    LV_VROUWEN_2019 = fmt(cijfers$lv_vrouwen_2019, 2),
    FIG1 = fig_to_html(fig1, "fig1", include_js = TRUE),
    FIG2 = fig_to_html(fig2, "fig2"),
    FIG3 = fig_to_html(fig3, "fig3"),
    FIG4 = fig_to_html(fig4, "fig4"),
    FIG5 = fig_to_html(fig5, "fig5"),
    FIG6 = fig_to_html(fig6, "fig6"),
    SCENARIO_JAAR = cijfers$scenario_jaar,
    SCENARIO_WERKELIJK = fmt(cijfers$scenario_werkelijk, 1),
    SCENARIO_VERWACHT = fmt(cijfers$scenario_verwacht, 1),
    SCENARIO_VERSCHIL = fmt(cijfers$scenario_verschil, 2),
    DATASETS_BRON = datasets$bron,
    CBS_BRON_URL = CBS_BRON_URL
  )
...
<div class="layout">
  ...
  <main class="main">
    ...
    <div class="hero">
      ...
      <p class="meta">
        Gepubliceerd {{VANDAAG}} &middot; cijfers: CBS &middot;
    ...
    <section class="chapter" id="inleiding">
      <h2>Inleiding</h2>
      <p>
        In {{LAATSTE_JAAR}} overleden er in Nederland
        {{OVERLEDENEN_LAATSTE}} duizend mensen, ongeveer
        {{OVERLEDENEN_VERSCHIL}} meer dan een jaar eerder.
      ...
      <div class="stat-row">
        <div class="stat"><span class="num">{{LV_MANNEN_LAATSTE}} jr</span><span class="label">levensverwachting mannen, {{LAATSTE_JAAR}}</span></div>
        <div class="stat"><span class="num">{{LV_VROUWEN_LAATSTE}} jr</span><span class="label">levensverwachting vrouwen, {{LAATSTE_JAAR}}</span></div>
        <div class="stat"><span class="num">{{OVERLEDENEN_LAATSTE}}k</span><span class="label">overledenen in {{LAATSTE_JAAR}}</span></div>
      </div>
    </section>
    ...
    <section class="chapter" id="overledenen">
      ...
      <div class="chart-frame">{{FIG1}}</div>
    ...
    <section class="chapter" id="leeftijd">
      ...
      <p>
        Van alle mensen die in {{LAATSTE_JAAR}} overleden was het
      ...
      <div class="chart-frame">{{FIG2}}</div>
    ...
    <section class="chapter" id="levensverwachting">
      ...
      <p>
        ...
        {{LAATSTE_JAAR}} inmiddels iets hoger dan vlak voor de
        pandemie ({{LV_MANNEN_2019}} jaar in 2019); voor vrouwen
        ligt het nog altijd iets lager dan in 2019
        ({{LV_VROUWEN_2019}} jaar).
      </p>
      <div class="chart-frame">{{FIG3}}</div>
    ...
    <section class="chapter" id="eu">
      ...
      <div class="chart-frame">{{FIG4}}</div>
    ...
    <section class="chapter" id="gestandaardiseerd">
      ...
      <div class="chart-frame">{{FIG5}}</div>
    ...
    <section class="chapter" id="scenario">
      ...
      <p>
        ...
        doorgetrokken naar 2020&ndash;{{LAATSTE_JAAR}}. Dat geeft een
        ruwe schatting van waar de sterfte had kunnen liggen als de
        trendbreuk van de pandemie niet had plaatsgevonden.
      </p>
      <div class="chart-frame">{{FIG6}}</div>
      <p class="caption">Eigen berekening: lineaire trend gefit op CBS-cijfers 2010&ndash;2019, doorgetrokken naar 2020&ndash;{{LAATSTE_JAAR}}.</p>
      <div class="callout own">
        <strong>Wat dit wel en niet zegt.</strong>
        In {{SCENARIO_JAAR}} ligt de gestandaardiseerde sterfte
        ({{SCENARIO_WERKELIJK}} per duizend) nog altijd
        {{SCENARIO_VERSCHIL}} punt boven wat de trend van voor de
        pandemie zou voorspellen ({{SCENARIO_VERWACHT}} per
    ...
    <section class="chapter" id="methode">
      ...
      <p>
        Databron voor deze run: <strong>{{DATASETS_BRON}}</strong>. Dit rapport
        ...
        staat bovenaan in <code>main.R</code> (<code>local &lt;- TRUE/FALSE</code>).
      ...
      <p>
        De verwerking gebeurt in R (base R voor databewerking en de
        lineaire trendfit met <code>lm()</code>, met alleen <code>jsonlite</code>
        en <code>httr</code> als externe dependencies). De volledige code
        staat in de bijbehorende repository: <code>main.R</code> als
        startpunt, en <code>utils/</code> met de losse stappen
        (<code>data_loader.R</code>, <code>analysis.R</code>,
        <code>visuals.R</code>, <code>report.R</code>, <code>parameters.R</code>).
      </p>
      <h3>Beperkingen</h3>
      <p>
    ...
    <footer>
      <p>
        ...
        <a href="{{CBS_BRON_URL}}" target="_blank" rel="noopener">
...
'
  ...
  render_template(html_template, vars)
}
